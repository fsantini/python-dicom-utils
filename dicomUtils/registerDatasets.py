...
def calcTransform(dataFix, dataFixInfo, dataMov, dataMovInfo, doElastix = True):
    ...
    MovWorldResolution = np.matmul(ijkMov, MovResolution)
    FixWorldResolution = np.matmul(ijkFix, FixResolution)
    WorldZoom = MovWorldResolution / FixWorldResolution
    ...
    print(dataMovInfo[0].ImagePositionPatient)
    print(dataFixInfo[0].ImagePositionPatient)
    
    print(MovCenterPointWorld)
    print(FixCenterPointWorld)
    
    #print(MovCenterPointWorld, FixCenterPointWorld)
    
    diffCenterWorld = (FixCenterPointWorld-MovCenterPointWorld)
    
    print(diffCenterWorld)
    
    MovCentralPointFix = np.matmul(ijkFix.T, (MovCenterPointWorld - dataFixInfo[0].ImagePositionPatient)/FixResolution)
    print(MovCentralPointFix)
    
    diffCenter = np.matmul(ijkFix.T, diffCenterWorld.T)/FixResolution
    #diffCenter = np.matmul(ijkFix.T, diffCenterWorld.T/FixWorldResolution)
    #diffCenter = MovCentralPointFix - FixCenterPoint
...
class DatasetTransform2D:
    ...
    def transform(self, datasetIn, twoDMode_unused = True):
        datasetOut = []
...
def calcTransform2DStack(dataFix_unused, dataFixInfo, dataMov_unused, dataMovInfo, doElastix_unused = False):
    assert not doElastix_unused, "Elastix registration cannot be used for 2D stack alignment"
